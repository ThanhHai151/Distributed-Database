...
function getSuggestions(errorMsg: string, sqlText: string): string[] {
  const hints: string[] = [];
...
function findErrorLine(errorMsg: string, sqlText: string): number | null {
  ...
  if (tokenMatch) {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].includes(tokenMatch[1])) return i + 1;
    }
  }
  return null;
}
...
export async function queryRoutes(app: FastifyInstance) {
  ...
  app.post<{
    ...
    // Allow DML by default. Allow DDL only if env says so.
    const allowDDL = process.env.ALLOW_DDL === 'true';
    // Remove SQL comments to correctly identify the first executable keyword
